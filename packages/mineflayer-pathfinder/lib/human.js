...
function createHuman (bot, opts = {}) {
  ...
  // took a step. The in-flight promise is handed out as is, so the setback guard rejects rather
  // than throws: a caller of the synchronous walkTo gets a rejected promise either way.
  function walkTo (goal, o = {}) {
    if (heldUntil > 0) return Promise.reject(new Error('setback'))
