...
Turns the head to `point` after a reaction delay and resolves once it has settled. Rejects with `setback` while `human.held` is true. Rejects straight away when `bot.physicsEnabled` is `false`, because the head only moves on a physics tick.
