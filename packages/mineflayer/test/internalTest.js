...
for (const supportedVersion of versionsUnderTest) {
  ...
  describe(`mineflayer_internal ${supportedVersion}v`, function () {
    ...
    describe('physics', () => {
      ...
      it('a teleport leaves the standing state to the physics tick', async function () {
        const client = (await once(server, 'playerJoin'))[0]
        await client.write('login', bot.test.generateLoginPacket())
        const chunk = bot.test.buildChunk()
        chunk.setBlockType(pos, goldId)
        ...
        // ClientPacketListener.handleMovePlayer only sets position, velocity and rotation; the
        // bot keeps standing until a tick of physics says otherwise.
        const moved = once(bot, 'forcedMove')
        await client.write('position', { ...onBlock, teleportId: 2 })
        await moved
        assert.strictEqual(bot.entity.onGround, true, 'still standing right after the teleport')
      })
